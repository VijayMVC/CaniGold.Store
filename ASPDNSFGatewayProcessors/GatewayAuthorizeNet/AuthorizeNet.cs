...
namespace AspDotNetStorefrontGateways.Processors
{
	...
	{
		public const string SolutionId = "AAA143870";

		public Boolean IsCimEnabled
		{
			get
			{
				return StringComparer.OrdinalIgnoreCase.Equals(AppLogic.AppConfig("PaymentGateway"), Gateway.AuthorizeNetGatewayName)
		...
		public override string CaptureOrder(Order o)
		{
			...
			if(useLiveTransactions)
				transactionCommand.AppendFormat("&x_solution_id={0}", SolutionId);
				...
				DB.ExecuteSQL(@"
					...
					values(
					 ...
					 new[]
					 {
						...
						new SqlParameter("@gateway", Gateway.AuthorizeNetGatewayName),
						new SqlParameter("@paymentMethod", AppLogic.TransactionTypeEnum.RECURRING_AUTO.ToString()),
						new SqlParameter("@command", RecurringSubscriptionCommand),
						new SqlParameter("@status", RecurringSubscriptionResult),
		...
		public override string ProcessAutoBillStatusFile(string gateway, string statusFile, out string results, RecurringOrderMgr orderManager)
		{
			var resultsBuilder = new StringBuilder();
			// Authorize.net provides two CSV files in an email. 
			try
			{
				var dt = CsvParser.Parse(statusFile, true);
				var separator = String.Empty;
				foreach(DataRow row in dt.Rows)
				{
					resultsBuilder.Append(separator);
					var subscriptionId = row["SubscriptionID"].ToString();
					var responseCode = row["RespCode"].ToString();
					var responseText = row["RespText"].ToString();
					...
					var originalOrderNumber = AppLogic.GetOriginalRecurringOrderNumberFromSubscriptionID(subscriptionId);

					// transaction date should be the date of the NextRecurringShipDate from the 
					// last order inserted, so that each order imported don't all get set to today's date
					var transactionDate = AppLogic.GetLastRecurringOrderDate(subscriptionId);

					resultsBuilder.Append("Processing ID ");
					resultsBuilder.Append(subscriptionId);
					resultsBuilder.Append(",");
					resultsBuilder.Append(responseCode);
					resultsBuilder.Append("=");
					...
					try
					{
						...
						if(transactionDate > DateTime.Today)
						{
							LogFailedTransaction(
								...
								gateway: gateway,
								status: "This order was already processed and is up to date",
								subscriptionId: subscriptionId,
								transactionId: transactionId);
						}
						else if(transactionDate == DateTime.MinValue)
						{
							// date will be minimum from GetLastRecurringOrderDate if no shopping cart record found
							LogFailedTransaction(
								customerId: 0,
								orderNumber: 0,
								ipAddress: "0.0.0.0",
								transactionDate: DateTime.Today,
								gateway: gateway,
								status: "Shopping Cart Record Not Found",
								subscriptionId: subscriptionId,
								transactionId: transactionId);
						}
						else if(originalOrderNumber == 0)
						{
							LogFailedTransaction(
								customerId: 0,
								orderNumber: 0,
								...
								gateway: gateway,
								status: "No Original Order Found",
								subscriptionId: subscriptionId,
								transactionId: transactionId);
						}
						else
						{
							...
							{
								int NewOrderNumber;
								status = orderManager.ProcessAutoBillApproved(originalOrderNumber, transactionId, transactionDate, out NewOrderNumber);
							}
							else
							{
								status = orderManager.ProcessAutoBillDeclined(originalOrderNumber, transactionId, transactionDate, subscriptionId, responseCode);
							}

							if(status != AppLogic.ro_OK)
							{
								var ProcessCustomerID = Order.GetOrderCustomerID(originalOrderNumber);
								var ProcessCustomer = new Customer(ProcessCustomerID, true);

								if(transactionDate.Equals(DateTime.MinValue))
									transactionDate = DateTime.Now;

								LogFailedTransaction(
									customerId: ProcessCustomer.CustomerID,
									orderNumber: originalOrderNumber,
									ipAddress: ProcessCustomer.LastIPAddress,
									transactionDate: transactionDate,
									gateway: gateway,
									status: status,
									subscriptionId: subscriptionId,
					...
					{
						resultsBuilder.Append(ex.Message);
					}
					separator = "\n";
				}
			}
		...
		void LogFailedTransaction(int customerId, int orderNumber, string ipAddress, DateTime transactionDate, string gateway, string status, string subscriptionId, string transactionId)
		{
			if(!string.IsNullOrEmpty(transactionId))
				status = string.Join(", ", status, "PNREF=" + transactionId);
