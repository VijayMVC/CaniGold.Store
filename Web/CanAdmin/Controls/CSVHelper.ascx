...
<script type="text/javascript">
	...
	function Reload<%= UniqueJSID %>() {
		var searchurl = 'x-ajax.csvhelper.aspx?&<%= EntityType.ToString() %>=entity&searchterm=' + encodeURIComponent(jQuery('#<%= UniqueJSID %>SearchBox').val()) + '&csvlist=,' + jQuery(<%= UniqueJSID %>targetTextBox).val() + ',';
		jQuery.ajax({
			url: searchurl,
