				...
				else
				{
					...
					else
					{
						...
						else
						{
							...
							writer.Append("<input onClick=\"AdHocOrderTypeChanged()\" type=\"radio\" value=\"" + AppLogic.TransactionTypeEnum.CREDIT.ToString() + "\" id=\"RefundOrderType\" name=\"OrderType\" checked>" + AppLogic.GetString("adhoccharge.aspx.18", ThisCustomer.SkinID, ThisCustomer.LocaleSetting));
							if(GW == Gateway.ro_GWACCEPTJS)
							{
								writer.Append("&nbsp;&nbsp;&nbsp;&nbsp;");
								writer.Append(AppLogic.GetString("adhoccharge.aspx.30", ThisCustomer.SkinID, ThisCustomer.LocaleSetting));
							}
							writer.Append("</td></tr>");
