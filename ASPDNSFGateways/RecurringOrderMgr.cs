...
namespace AspDotNetStorefrontGateways.Processors
{
	...
	public class RecurringOrderMgr
	{
		...
		public String ProcessRecurringOrder(int OriginalRecurringOrderNumber, string XID, DateTime PaymentTime, out int NewOrderNumber)
		{
			...
			if(cart.CartItems.Count != 0 && originalOrder.PaymentMethod != AppLogic.ro_PMPayPalExpress)
			{
				...
				if(status == AppLogic.ro_OK)
				{
					...
					UpdateOrderFromOriginal(OrderNumber, originalOrder);
		...
		void UpdateOrderFromOriginal(int orderNumber, Order originalOrder)
		{
			// copy order totals and shipping information from original order
			// This needs to be done because there isn't a cart context in Prices calculations, and so they come back zero, 
			// and PrimaryShippingAddressID and PrimaryBillingAddressID are used to create the new order - but they aren't updated (and shouldn't be) 
			// when processing a gateway recurring order

			var orderUpdateParams = new[] {
				new SqlParameter("@OrderNumber", orderNumber),
				new SqlParameter("@OriginalOrderNumber", originalOrder.OrderNumber)
				};

			DB.ExecuteSQL(@"
				update NewOrder set 
				OrderShippingCosts = origOrder.OrderShippingCosts,
				OrderTax = origOrder.OrderTax,
				OrderTotal = origOrder.OrderTotal,
				
				ShippingAddress1 = origOrder.ShippingAddress1,
				ShippingAddress2 = origOrder.ShippingAddress2,
				ShippingCity = origOrder.ShippingCity,
				ShippingState = origOrder.ShippingState,
				ShippingZip = origOrder.ShippingZip,
				ShippingCountry = origOrder.ShippingCountry,
				ShippingFirstName = origOrder.ShippingFirstName,
				ShippingLastName = origOrder.ShippingLastName,
				ShippingCompany = origOrder.ShippingCompany,
				ShippingSuite = origOrder.ShippingSuite,
				ShippingResidenceType = origOrder.ShippingResidenceType,
				ShippingPhone = origOrder.ShippingPhone,
				
				BillingAddress1 = origOrder.BillingAddress1,
				BillingAddress2 = origOrder.BillingAddress2,
				BillingCity = origOrder.BillingCity,
				BillingState = origOrder.BillingState,
				BillingZip = origOrder.BillingZip,
				BillingCountry = origOrder.BillingCountry,
				BillingFirstName = origOrder.BillingFirstName,
				BillingLastName = origOrder.BillingLastName,
				BillingCompany = origOrder.BillingCompany,
				BillingSuite = origOrder.BillingSuite,
				BillingPhone = origOrder.BillingPhone

				from orders NewOrder
				join orders OrigOrder on OrigOrder.OrderNumber = @OriginalOrderNumber
				where NewOrder.OrderNumber = @OrderNumber", orderUpdateParams);
		}
