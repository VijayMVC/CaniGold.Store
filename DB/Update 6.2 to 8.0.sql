...
/* *** ASPDOTNETSTOREFRONT v6.2.x.x to v8.0 DB update [dbo].SCRIPT *** */
/*                                                                     */
/*                                                                     */
...
/*                                                                     */
/*                                                                     */
/* Run this script to update ANY 6.2.x.x version to the latest version */
/*                                                                     */
/* ******************************************************************* */
...
/**********************************  table mods and creates  **********************************/
    ...
    /** drop and recreate CustmerSession Table  **/
    if exists (select * From sysobjects where id = object_id('CustomerSession') and type = 'u') begin
    ...
    if not exists (select * From syscolumns where id = object_id('StateTaxRate') and name = 'StateTaxID')  BEGIN
        if exists (select * from sysobjects where name = 'DF_StateTaxRate_TaxClassID' and type = 'D')
            ALTER TABLE dbo.StateTaxRate DROP CONSTRAINT DF_StateTaxRate_TaxClassID
        if exists (select * from sysobjects where name = 'DF_StateaxRate_CreatedOn' and type = 'D')
            ALTER TABLE dbo.StateTaxRate DROP CONSTRAINT DF_StateTaxRate_CreatedOn
        ALTER TABLE dbo.StateTaxRate DROP CONSTRAINT PK_StateTaxRate


        CREATE TABLE [dbo].[StateTaxRate_new](
            [StateTaxID] int NOT NULL IDENTITY,
            [StateID] [int] NOT NULL,
            [TaxClassID] [int] NOT NULL CONSTRAINT DF_StateTaxRate_TaxClassID DEFAULT((1)),
            [TaxRate] [money] NULL,
        ...
        INSERT dbo.StateTaxRate_new (StateID, TaxClassID, TaxRate, CreatedOn) 
        SELECT StateID, TaxClassID, TaxRate, CreatedOn FROM dbo.StateTaxRate

        DROP TABLE dbo.StateTaxRate

        exec sp_rename 'StateTaxRate_new', 'StateTaxRate'
        CREATE UNIQUE INDEX [UIX_StateTaxRate_StateID_TaxClassID] ON [StateTaxRate] (StateID,TaxClassID) 
    END
    go
    ...
    if not exists (select * From syscolumns where id = object_id('CountryTaxRate') and name = 'CountryTaxID')  BEGIN
        if exists (select * from sysobjects where name = 'DF_CountryTaxRate_TaxClassID' and type = 'D')
            ALTER TABLE dbo.CountryTaxRate DROP CONSTRAINT DF_CountryTaxRate_TaxClassID
        if exists (select * from sysobjects where name = 'DF_CountryaxRate_CreatedOn' and type = 'D')
            ALTER TABLE dbo.CountryTaxRate DROP CONSTRAINT DF_CountryTaxRate_CreatedOn
        ALTER TABLE dbo.CountryTaxRate DROP CONSTRAINT PK_CountryTaxRate


        CREATE TABLE [dbo].[CountryTaxRate_new](
            [CountryTaxID] int NOT NULL IDENTITY,
            [CountryID] [int] NOT NULL,
            [TaxClassID] [int] NOT NULL CONSTRAINT DF_CountryTaxRate_TaxClassID DEFAULT((1)),
            [TaxRate] [money] NULL,
        ...
        INSERT dbo.CountryTaxRate_new (CountryID, TaxClassID, TaxRate, CreatedOn) 
        SELECT CountryID, TaxClassID, TaxRate, CreatedOn FROM dbo.CountryTaxRate

        DROP TABLE dbo.CountryTaxRate

        exec sp_rename 'CountryTaxRate_new', 'CountryTaxRate'
        CREATE UNIQUE INDEX [UIX_CountryTaxRate_CountryID_TaxClassID] ON [CountryTaxRate] (CountryID,TaxClassID)
    END
    go
    ...
    IF not exists (select * From syscolumns where id = object_id('ZipTaxRate') and name = 'ZipTaxID') BEGIN
        ...
        if exists (select * from sysobjects where name = 'DF_ZipTaxRate_TaxClassID' and type = 'D')
    ...
    -- taxshipping is now set on the shipping tax class id by country, state, or zip
    if exists (select * from syscolumns where id = object_id('State')  and name = 'TaxShipping' and cdefault <> 0)
        exec aspdnsf_DropColumnWithDefaultConstraint 'State','TaxShipping';
    go

    if exists (select * from syscolumns where id = object_id('Country')  and name = 'TaxShipping' and cdefault <> 0)
        exec aspdnsf_DropColumnWithDefaultConstraint 'Country','TaxShipping';
    go
    ...
    if not exists (select * From syscolumns where id = object_id('Customer') and name = 'VATSetting') begin
        alter table dbo.Customer add [VATSetting] int NOT NULL CONSTRAINT DF_customer_VATSetting default(0), -- 1 = show prices inclusive of VAT, 2 = show prices exclusive of VAT
    ...
    if not exists (select * From syscolumns where id = object_id('Orders_ShoppingCart') and name = 'CustomerEntersPricePrompt')
    ...
    if not exists (select * From syscolumns where id = object_id('ShoppingCart') and name = 'TaxClassID') 
        alter table dbo.ShoppingCart add [TaxClassID] [int] not null CONSTRAINT DF_ShoppingCart_TaxClassID DEFAULT(1)
    go
    if not exists (select * From syscolumns where id = object_id('ShoppingCart') and name = 'TaxRate') 
        alter table dbo.ShoppingCart add [TaxRate] [money] not null CONSTRAINT DF_ShoppingCart_TaxRate DEFAULT(0)
    go
    ...
    if exists (select * From syscolumns where id = object_id('Orders') and name = 'StateTaxRate')  BEGIN
       DECLARE @cmd varchar(8000) 
    ...
    if exists (select * From syscolumns where id = object_id('Country') and name = 'TaxRate')  begin
        ...
        if exists (select * from sysobjects where name = 'DF_Country_TaxRate' and type = 'D')
            alter table dbo.Country drop constraint DF_Country_TaxRate
        SET @cmd = 'alter table dbo.Country drop column TaxRate'
        EXEC (@cmd) 
    end

    go

    if exists (select * From syscolumns where id = object_id('Orders') and name = 'Password')  begin
        alter table orders drop column password
    end
    go
    ...
    if not exists (select * From syscolumns where id = object_id('EventHandler') and name = 'Debug') begin
        ALTER TABLE dbo.Orders ADD Debug bit not null CONSTRAINT DF_EventHandler_Debug DEFAULT (0)
    end 
    go
...
/****************************************   Data    *******************************************/
    ...
    -- set new default on this AppConfig for backwards compatibility:
    update AppConfig set ConfigValue='OrderNumber,OrderDate,OrderTotal,IsNew,CustomerID,Password,FirstName,LastName,Email,Notes,Phone,AffiliateID,CardType,CardName,CardNumber,Last4,CardExpirationMonth,CardExpirationYear,PaymentMethod,OrderNotes,FinalizationData,PONumber,DownloadEmailSentOn,ReceiptEmailSentOn,ShippingTrackingNumber,ShippedVIA,AVSResult,LocaleSetting' where Name='OrderSummaryReportFields' and (ConfigValue='' or ConfigValue IS NULL)
    go
    ...
    if not exists(select * from dbo.AppConfig where name = 'Checkout.UseOnePageCheckout')
        INSERT dbo.AppConfig(SuperOnly,Name,GroupName,Description,ConfigValue) values(0,'Checkout.UseOnePageCheckout','GENERAL','Use single page checkout instead of normal multi-step checkout','false')
    go
    if not exists(select * from dbo.AppConfig where name = 'Checkout.UseOnePageCheckout.UseFinalReviewOrderPage')
        INSERT dbo.AppConfig(SuperOnly,Name,GroupName,Description,ConfigValue) values(0,'Checkout.UseOnePageCheckout.UseFinalReviewOrderPage','GENERAL','IF using one page checkout, and IF your order qualifies for one page checkout, this flag indicates whether to take the user to a final "review order" page before sumitting the transaction. if false, they submit the transaction right on the one page checkout page. We do recommend this being true, to allow the customer to review their full final order stats before "charging" them. Some card processors/merchant accounts/ banks may also require this step!','true')
    go

    if not exists(select * from dbo.AppConfig where name = 'ShowCardStartDateFields')
        INSERT [dbo].AppConfig(SuperOnly,Name,GroupName,Description,ConfigValue) values(0,'ShowCardStartDateFields','MISC','if true, credit card forms will show the Card Start Date, and Card Issue Number fields, used for UK/EU customers.','false');
    go
    ...
    if not exists(select * from dbo.AppConfig where name = 'DistributorEMailCC')
        INSERT [dbo].AppConfig(SuperOnly,Name,GroupName,Description,ConfigValue) values(0,'DistributorEMailCC','GENERAL','If you want a copy of distributor notification e-mails sent to another e-mail address (e.g. your e-mail address), enter that e-mail address here','')
    go
    ...
    if exists(select * from dbo.AppConfig where name = 'EncryptPassword')
    ...
    Update AppConfig Set ConfigValue='15' where Name='SessionTimeoutInMinutes';
    go
    ...
    if not exists(select * from dbo.AppConfig where name = 'GoogleCheckout.BaseUrl')
        INSERT [dbo].AppConfig(SuperOnly,Name,GroupName,Description,ConfigValue) values(0,'GoogleCheckout.BaseUrl','GOOGLE CHECKOUT','Don''t change this value. It''s the URL that your website uses to communicate with Google Checkout.','https://checkout.google.com/api/checkout/v2/request/Merchant/{0}');
    go
    ...
    if not exists(select * from dbo.AppConfig where name = 'DynamicRelatedProducts.Enabled')
        insert dbo.AppConfig(SuperOnly, Name, GroupName, Description, ConfigValue) values(0, 'DynamicRelatedProducts.Enabled','DISPLAY','When set to true, the products that are related to the product currently viewed by the customer will be displayed at the bottom of the Product page. Valid values are true and false only','true')
    GO

    if not exists(select * from dbo.AppConfig where name = 'DynamicRelatedProducts.NumberDisplayed')
        insert dbo.AppConfig(SuperOnly, Name, GroupName, Description, ConfigValue) values(0, 'DynamicRelatedProducts.NumberDisplayed','DISPLAY','Controls the maximum number of related products that will be displayed at the bottom of the Product pages. Valid values ranges from 0 - 99999999','4')
    GO
    ...
    update dbo.AppConfig set ConfigValue='OrderNumber,OrderDate,OrderTotal,IsNew,CustomerID,FirstName,LastName,Email,Notes,Phone,AffiliateID,CardType,CardName,CardNumber,Last4,CardExpirationMonth,CardExpirationYear,PaymentMethod,OrderNotes,FinalizationData,PONumber,DownloadEmailSentOn,ReceiptEmailSentOn,ShippingTrackingNumber,ShippedVIA,AVSResult,LocaleSetting' where Name='OrderSummaryReportFields';
    go
...
-- RTShipping Local Pickup appconfigs
    ...
    -- Update customer records to set the isadmin field to 3 for SuperAdmins.  The IsAdmin field is now a bitmask field where 1 = Admin, 3 = SuperAdmin 
