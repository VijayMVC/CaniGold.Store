...
namespace AspDotNetStorefrontCore
{
	...
	public class DownloadItem
	{
		...
		public void SendDownloadEmailNotification(bool ignoreMaxMind)
		{
			var order = new Order(OrderNumber);
			var maxMindEnabled = AppLogic.AppConfigBool("MaxMind.Enabled");
			var isMaxMindFlaggedFraud = order.MaxMindFraudScore >= AppLogic.AppConfigNativeDecimal("MaxMind.DelayDownloadThreshold");

			// Don't send download email if MaxMind is enabled and this order's MaxMind fraud score exceeds the set threshold
			// Also ignore Max Mind if an Admin manually triggers this
			if(maxMindEnabled
				&& isMaxMindFlaggedFraud
				&& !ignoreMaxMind)
				return;
