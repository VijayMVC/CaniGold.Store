...
{
	public class SkinImage : IParameterizedTokenHandler
	{
		const string NameParam = "name";

		readonly string[] Tokens = { "skinimage" };
		readonly string[] ParameterNames = { NameParam };

		readonly TokenParameterConverter TokenParameterConverter;

		public SkinImage(TokenParameterConverter tokenParameterConverter)
		{
		...
		public string RenderToken(TokenHandlerContext context)
		{
			...
			var imageName = context
				.Parameters[NameParam]
				.Trim();

			return AppLogic.SkinImage(imageName);
		}
