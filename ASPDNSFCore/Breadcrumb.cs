	...
	public class Breadcrumb
	{
		...
		public static String GetProductBreadcrumb(int ProductID, String ProductName, String SourceEntity, int SourceEntityID, Customer ThisCustomer)
		{
			...
			if(SourceEntityID != 0)
			{
				...
				while(ThisID != 0)
				{
					st.Insert(0, String.Format("<a class=\"SectionTitleText\" href=\"{0}\">{1}</a> {2} ", Url.BuildEntityLink(SourceEntity, ThisID, string.Empty), hlp.GetEntityName(ThisID, ThisCustomer.LocaleSetting), separator));
		...
		public static String GetEntityBreadcrumb(int EntityID, String EntityInstanceName, String SourceEntity, Customer ThisCustomer)
		{
			...
			while(ParentID != 0)
			{
				st.Insert(0, String.Format("<a class=\"SectionTitleText\" href=\"{0}\">{1}</a> {2} ", Url.BuildEntityLink(SourceEntity, ParentID, string.Empty), hlp.GetEntityName(ParentID, ThisCustomer.LocaleSetting), separator));
