...
namespace AspDotNetStorefrontCore
{
	...
	public partial class RTShipping
	{
		...
		private void DHLIntlGetRates(Shipments allShipments, out string rtShipRequest, out string rtShipResponse, decimal ExtraFee, decimal markupPercent, decimal shippingTaxRate, ref ShippingMethodCollection shippingMethods)
		{
			...
			if(ShipmentWeight > AppConfigProvider.GetAppConfigValueUsCulture<decimal>("RTShipping.DHLIntl.MaxWeight", StoreId, true))
			{
			...
			string dhlServer;
			if(AppConfigProvider.GetAppConfigValue<bool>("RTShipping.UseTestRates", StoreId, true))
			{
				dhlServer = AppConfigProvider.GetAppConfigValue("RTShipping.DHL.TestServer", StoreId, true);
			}
			else
			{
				dhlServer = AppConfigProvider.GetAppConfigValue("RTShipping.DHL.Server", StoreId, true);
			}
			...
			do
			{
				...
				foreach(Packages shipment in allShipments)
				{
					...
					foreach(Package p in shipment)
					{
						...
						rateRequests.Requestor.ID = AppConfigProvider.GetAppConfigValue("RTShipping.DHL.APISystemID", StoreId, true);
						rateRequests.Requestor.Password = AppConfigProvider.GetAppConfigValue("RTShipping.DHL.APISystemPassword", StoreId, true);

						string dhlServices = AppConfigProvider.GetAppConfigValue("RTShipping.DHLIntl.Services", StoreId, true);
						...
						foreach(string service in dhlServices.Split(','))
						{
							...
							request.ShippingCredentials.ShippingKey = AppConfigProvider.GetAppConfigValue("RTShipping.DHLIntl.ShippingKey", StoreId, true);
							request.ShippingCredentials.AccountNbr = AppConfigProvider.GetAppConfigValue("RTShipping.DHL.AccountNumber", StoreId, true);

							// shipment details
							request.ShipmentDetail.ShipDate = shipDate.ToString("yyyy-MM-dd");
							request.ShipmentDetail.ShipmentType.Code = AppConfigProvider.GetAppConfigValue("RTShipping.DHLIntl.Packaging", StoreId, true).ToUpperInvariant();
							...
							// billing details
							request.Billing.Party.Code = AppConfigProvider.GetAppConfigValue("RTShipping.DHLIntl.BillingParty", StoreId, true).ToUpperInvariant();
							if(request.Billing.Party.Code != "S") request.Billing.AccountNbr = AppConfigProvider.GetAppConfigValue("RTShipping.DHLIntl.BillingAccountNbr", StoreId, true).ToUpperInvariant();

							request.Billing.DutyPaymentType = AppConfigProvider.GetAppConfigValue("RTShipping.DHLIntl.DutyPayment", StoreId, true).ToUpperInvariant();
							if(request.Billing.DutyPaymentType == "3") request.Billing.DutyPaymentAccountNbr = AppConfigProvider.GetAppConfigValue("RTShipping.DHLIntl.DutyPaymentAccountNbr", StoreId, true).ToUpperInvariant();

							// import duty declaration
							request.Dutiable.DutiableFlag = AppConfigProvider.GetAppConfigValue("RTShipping.DHLIntl.Dutiable", StoreId, true).ToUpperInvariant();
							request.Dutiable.CustomsValue = p.InsuredValue.ToString("######");

							// overrides
							string overrideCodes = AppConfigProvider.GetAppConfigValue("RTShipping.DHLIntl.Overrides", StoreId, true);
	...
	{
		[XmlRoot("ECommerce")]
		public partial class InternationalRateRequest
		{
		...
		[XmlRoot("ECommerce")]
		public partial class InternationalRateResponse
		{
