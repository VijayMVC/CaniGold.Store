...
namespace AspDotNetStorefrontCore
{
	...
	public partial class RTShipping
	{
		public ShippingMethodCollection GetRates(
			Shipments allShipments,
			decimal shipmentWeight,
			decimal shipmentValue,
			decimal shippingTaxRate,
			out string rtShipRequest,
			out string rtShipResponse,
			ref ups2.ShippingSection shippingConfig,
			ref ups2.UPSSection upsConfig)
		{
			...
			foreach(Packages Shipment in allShipments)
			{
				...
				foreach(string service in upsConfig.Services.Split(','))
				{
					...
					foreach(ups2.ResponseRatedShipment ratedShipment in response.RatedShipment)
					{
						...
						if(total == 0 || serviceDescription == string.Empty || !ShippingMethodIsAllowed(serviceDescription, RTShipping.UpsName))
							continue;
