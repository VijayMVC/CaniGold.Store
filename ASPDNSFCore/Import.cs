...
namespace AspDotNetStorefrontCore
{
	...
	public class Import
	{
		...
		static Dictionary<Int32, string> RelatedProductsToDoList;
		static string ProcessingNode = string.Empty;
		...
		public static String ProcessXmlImportFile(String XmlFile)
		{
			...
			Import.ImportXML(tmpS, XmlContents, false);
		...
		private static void LoadProduct(StringBuilder StatusList, XmlNode node, bool FirstProductRow)
		{
			...
			else
			{
				...
				if(FirstProductRow)
				{
					...
					{
						var relatedNode = relatedList[i];
						if(relatedNode.InnerText.Length != 0)
						{
							var text = relatedNode.InnerText + (i < relatedList.Count - 1 ? "|" : "");
							if(i > 0)
								RelatedProductsToDoList[ProductID] += text;
							else
								RelatedProductsToDoList.Add(ProductID, text);
						}
					}
				}
		...
		public static void ImportXML(StringBuilder StatusList, String XMLContents, bool CleanOnError)
		{
			try
			{
				RelatedProductsToDoList = new Dictionary<int, string>();
				...
				foreach(XmlNode node in nodeList)
				{
					...
					if(!productImportIsRestricted)
					{
						...
						var nodeName = XmlCommon.XmlField(node, "Name");
						LoadProduct(StatusList, node, !lastProductName.EqualsIgnoreCase(nodeName));
						lastProductName = nodeName;
					}
				}

				// process relatedproducts (deferred):
				foreach(KeyValuePair<Int32, string> relatedProducts in RelatedProductsToDoList)
				{
					try
					{
						var thisPID = relatedProducts.Key;
						var products = relatedProducts.Value.Split('|');
						var update = "";
						foreach(string product in products)
						{
							var relPID = CheckForProduct(product);
							if(relPID != 0 && thisPID != relPID)
								update += relPID.ToString() + (product != products[products.GetUpperBound(0)] ? "," : "");
							else
								StatusList.Append("Ignoring unknown related product...");
						}
						if(update != "")
							RunCommand(StatusList, "update product set relatedproducts=" + DB.SQuote(update) + " where productid=" + thisPID);
					}
					catch
					{
						StatusList.Append("Ignoring unknown related product...");
